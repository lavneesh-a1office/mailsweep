...
export default function CategoryPreviewPage() {
  ...
  const filteredEmails = useMemo(() => {
    ...
    return categorizedEmails.filter(
      (email) => email.category === decodedCategory
    );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await setDoc(
        userDocRef,
        { categorizedEmails: remainingEmails, updatedAt: new Date() },
        { merge: true }
      );
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl md:text-3xl font-bold font-headline">
              Emails in "{decodedCategory}"
            </h1>
            <p className="text-muted-foreground">
              {filteredEmails.length.toLocaleString()} emails found
            </p>
          </div>
        </div>
      </header>
      ...
      {filteredEmails.length > 0 ? (
            ...
            {filteredEmails.map((email) => (
              <div key={email.id} className="flex items-center gap-2">
                <Checkbox
                  ...
                  className="ml-2"
                />
                <Card className="flex-grow bg-card hover:bg-accent/20 transition-colors">
                  <CardHeader
        ...
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground mt-2">
            There are no emails in the "{decodedCategory}" category.
          </p>
          <p className="text-muted-foreground mt-1">
            It's possible you haven't scanned your inbox yet or they have been
            deleted.
          </p>
          <Link href="/dashboard" passHref>
