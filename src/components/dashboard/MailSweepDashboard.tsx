...
import type {
  CategorizeEmailsInput,
  CategorizeEmailsOutput,
} from "@/ai/flows/categorize-emails";
import {
  Card,
  CardTitle,
  CardHeader,
...
export default function MailSweepDashboard({
    ...
    async (options: { forceRescan?: boolean; pageToken?: string }) => {
      ...
      if (!user) {
        toast({
          ...
          description: "Please login to fetch emails.",
          variant: "destructive",
        });
        router.push("/");
        return;
      ...
      if (!forceRescan && !pageToken) {
        ...
        if (docSnap.exists()) {
          ...
          if (data.categorizedEmails && data.categorizedEmails.length > 0) {
            setCategorizedEmails(
              data.categorizedEmails.map((e: any) => ({
                ...e,
            ...
            toast({
              title: "Success",
              description: "Loaded cached scan results.",
            });
      ...
      try {
        ...
        if (!accessToken) {
          toast({
            ...
            description:
              "Access token not found. Please log in again to grant permission.",
            variant: "destructive",
          });
          await handleLogout();
          return;
        ...
        const { emails: fetchedEmails, nextPageToken: newNextPageToken } =
          await fetchEmails({ accessToken, pageToken });
        ...
        if (fetchedEmails.length > 0) {
          toast({
            title: "Success",
            description: `Found ${fetchedEmails.length} emails to scan.`,
          });
          await handleStartScan(
            [...currentEmails, ...fetchedEmails],
            !!pageToken,
            newNextPageToken
          );
        } else {
          toast({
            ...
            description: `We couldn't find any more emails to scan.`,
          });
        }
      } catch (error) {
        ...
        toast({
          ...
          description:
            "Could not retrieve emails. Your session might have expired. Please log in again.",
          variant: "destructive",
        });
        await handleLogout();
      } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const remainingEmails = categorizedEmails.filter(
      ...
      toast({
        ...
        description:
          "Could not delete emails. Your session might have expired. Please log in again.",
        variant: "destructive",
      });
      await handleLogout();
    } finally {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center text-center p-4 py-16 bg-background">
        ...
        <h1 className="text-xl font-bold font-headline mt-4 mb-1">
          Scanning your inbox...
        </h1>
  ...
  if (totalEmailsCategorized === 0 && !isCategorizing && !isFetchingEmails) {
    return (
      <div className="flex flex-col items-center justify-center text-center p-4 py-16 bg-background">
        ...
        <h1 className="text-xl font-bold font-headline mb-2">
        ...
        <Button
          size="lg"
          onClick={() => handleFetchEmails({ forceRescan: true })}
  ...
  if ((isCategorizing || isFetchingEmails) && categorizedEmails.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center text-center p-4 py-16 bg-background">
        ...
        <h1 className="text-xl font-bold font-headline mt-4 mb-2">
          {isFetchingEmails
            ? "Fetching emails..."
        ...
        <Progress
          value={isFetchingEmails ? undefined : progress}
                ...
                <CardContent className="p-4 flex items-center justify-between gap-4">
                  <div className="flex-grow">
                    ...
                    <p className="text-xs text-muted-foreground">
                      We scan 500 at a time. Click to scan for more.
                    </p>
                  </div>
                  <Button
                    size="sm"
                    ...
                    onClick={() =>
                      handleFetchEmails({ pageToken: nextPageToken })
                    }
                    disabled={isFetchingEmails || isCategorizing}
                  >
                    <ScanSearch className="mr-2 h-4 w-4" />
                    {isFetchingEmails || isCategorizing
                      ? "Scanning..."
                      : "Scan More"}
                  </Button>
                </CardContent>
            ...
            <div className="fixed bottom-0 left-0 right-0 z-10 bg-background/95 p-4 backdrop-blur-sm border-t md:static md:bg-transparent md:p-0 md:border-none md:shadow-none">
              <Card className="md:shadow-lg md:bg-card">
                <CardHeader className="hidden md:flex">
                  ...
                  <CardDescription>
                    Review the emails that will be deleted based on your
                    selections.
                  </CardDescription>
                </CardHeader>
                  ...
                  <div className="flex md:flex-col items-center justify-between gap-4 md:space-y-4">
                    ...
                    >
                      ...
                      <span className="hidden sm:inline">
                        {isDeleting
                          ? "Deleting..."
                          : `Delete ${filteredEmailCount.toLocaleString()} Emails`}
                      </span>
